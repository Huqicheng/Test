...
//  Threads
//
//  Created by Elaine on 2017/11/21.
...
void* ApproxVc2(void *graph) {
    ...
    for (int i = 0; i < v; ++i) {
        for (int j = 0; j < v; ++j) {
            if (arc[i][j] == 1) {
                for (int k = 0; k < v; ++k) {
                    if ((vertexCover[k] == i) || (vertexCover[k] == j)) {
                        flagIn = 1;
                        break;
                    }
                    else {
                ...
                if (flagIn == 2) {
                    vertexCover[count1] = i;
                    count1++;
                    vertexCover[count1] = j;
                    count1++;
                    for (int k = 0; k < v; ++k) {
                        arc[i][k] = 0;
                        arc[k][i] = 0;
    ...
    std::sort(vertexCover, vertexCover + count1);
